...
@section('title', 'Remove Last Characters - WordFix')

@section('tool-title', 'Remove Last Characters')

@section('tool-description', 'Remove last N characters from each line or entire text')

@section('tool-content')
<!-- Compact Configuration -->
<div class="mb-4 bg-rose-50 border border-rose-200 rounded-lg p-3">
    <div class="flex flex-wrap items-center gap-3">
        <div class="flex-1 min-w-[150px]">
            ...
            <input type="number" id="charCount" value="1" min="0" max="1000"
                class="w-full px-3 py-2 text-sm border border-rose-300 rounded focus:ring-2 focus:ring-rose-500"
                oninput="removeLastChars()">
        </div>
        
        <div class="flex flex-wrap gap-2">
            <label class="flex items-center cursor-pointer bg-white border border-rose-200 rounded px-3 py-1.5">
                <input type="checkbox" id="perLine" class="w-4 h-4 text-rose-600 rounded" checked onchange="removeLastChars()">
                <span class="ml-1.5 text-xs font-medium whitespace-nowrap">Per Line</span>
            </label>
            <label class="flex items-center cursor-pointer bg-white border border-rose-200 rounded px-3 py-1.5">
                <input type="checkbox" id="skipEmpty" class="w-4 h-4 text-rose-600 rounded" onchange="removeLastChars()">
                <span class="ml-1.5 text-xs font-medium whitespace-nowrap">Skip Empty Lines</span>
            </label>
            <label class="flex items-center cursor-pointer bg-white border border-rose-200 rounded px-3 py-1.5">
                <input type="checkbox" id="trimAfter" class="w-4 h-4 text-rose-600 rounded" onchange="removeLastChars()">
...
<x-text-converter 
    toolId="removeLastChars"
    inputPlaceholder="Paste your text here (one line per entry)..."
    outputPlaceholder="Text with last characters removed will appear here..."
...
<div id="statsSection" class="hidden mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
    <div class="bg-rose-50 border border-rose-200 rounded-lg p-3 text-center">
        <div class="text-xl font-bold text-rose-600" id="originalChars">0</div>
...
<article class="prose prose-blue max-w-none">
    <h2 class="text-2xl font-bold text-gray-900 mb-4">About Remove Last Characters</h2>
    <div class="text-gray-700 leading-relaxed space-y-4">
        <p>Remove a specified number of characters from the end of each line or the entire text. Perfect for cleaning data, removing suffixes, or text formatting.</p>
        
        <h3 class="text-xl font-semibold text-gray-900 mt-4 mb-2">Features</h3>
        <ul class="list-disc list-inside space-y-1 ml-4">
            <li>Specify number of characters to remove from end</li>
...
@push('scripts')
<script>
    function removeLastChars() {
        const input = document.getElementById('removeLastChars-input');
        const output = document.getElementById('removeLastChars-output');
        ...
        if (perLine) {
            ...
            result = lines.map(line => {
                ...
                linesProcessed++;
                const removeCount = Math.min(charCount, line.length);
                charsRemoved += removeCount;
                let processed = line.substring(0, line.length - removeCount);
            ...
            result = text.substring(0, text.length - charCount);
    ...
    setRemoveLastCharsConverter(text => {
        removeLastChars();
        return document.getElementById('removeLastChars-output').value;
    });
</script>
@endpush
